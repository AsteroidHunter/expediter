...
// /api/detach is the swipe-and-hold detach endpoint. As with /api/focus and
// /api/attach, only the body-validation and gone-pane branches are testable
// without a live tmux server; the happy path (detach-client) is manual.
function makeRequest(body: unknown, asString = false): RequestEvent {
	const request = new Request('http://localhost/api/detach', {
		method: 'POST',
...
// detachSession rejects ids that don't match /^%[0-9]+$/ with a FocusError; the
// route maps FocusError → 410 (Gone), same contract as /api/focus and /api/attach.
