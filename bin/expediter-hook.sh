...
#                 custom-title line; a Codex session uses its explicit thread
#                 name from session_index.jsonl or threads.name (stdlib
#                 sqlite3, read-only). The POST goes to localhost:5179 exactly
#                 as in local mode; the installer-written RemoteForward tunnel
...
# the transcript_path segment: /.codex/ reads the latest explicit thread_name
# from session_index.jsonl, falling back to threads.name in the box's state db
